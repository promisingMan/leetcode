...
public class DecodeWaysIi_639 {
    ...
    public static void main(String[] args) {
        Solution solution = new DecodeWaysIi_639().new Solution();
    }
    ...
    class Solution {
        ...
        public int numDecodings(String s) {
            ...
            dp[0] = 1;
            for (int i = 1; i <= n; i++) {
                dp[i] = dp[i - 1] * useOneChar(s.charAt(i - 1)) % MOD;
                if (i > 1) {
                    dp[i] += dp[i - 2] * useTwoChar(s.charAt(i - 2), s.charAt(i - 1)) % MOD;
                }
            }
            return (int) dp[n];
        }

        public int useOneChar(char c) {
            if (c == '0') {
                return 0;
            }
            return c == '*' ? 9 : 1;
        ...
        public int useTwoChar(char c1, char c2) {
            // 对应的有效编码 [11,19] 以及 [21,26]
            if (c1 == '*' && c2 == '*') {
                return 15;
            }
            if (c1 == '*') {
